  ...
  async function attach(opts) {
    ...
    const onCdpEvent = (source, method, params) => {
      ...
      if (method === 'Network.responseReceived') {
        ...
        // CDP Network.* timestamps are MonotonicTime (seconds since an arbitrary
        // origin), not epoch ms — so stamp with Date.now() at receipt (emitNetwork
        // defaults t to Date.now() when omitted). Network.enable doesn't replay
        // pre-session requests, so the `since` filter isn't needed here.
        emitNetwork(params.requestId, resp.status, resp.url);
        return;
      }
      if (method === 'Network.loadingFailed') {
        emitNetwork(params.requestId, params.errorText, null);
        return;
      }
