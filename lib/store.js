...
(function () {
  ...
  //   network   autoIncrement         { t, route, method, url, status }  (failures only)
  //   environment autoIncrement       { viewport, dpr, zoom, ua, host, url, capturedAt, custom }  (one record)
  //   annotations autoIncrement       { name, dataUrl, t, route }  (operator-drawn *_annotated.png)
  function open() {
    return new Promise((resolve, reject) => {
      ...
      req.onupgradeneeded = () => {
        ...
        if (!db.objectStoreNames.contains('network')) {
          db.createObjectStore('network', { autoIncrement: true });
        }
        if (!db.objectStoreNames.contains('environment')) {
          db.createObjectStore('environment', { autoIncrement: true });
        }
        if (!db.objectStoreNames.contains('annotations')) {
          db.createObjectStore('annotations', { autoIncrement: true });
        }
      };
  ...
  async function clearAll() {
    const db = await open();
    try {
      const names = ['frames', 'timeline', 'steps', 'narration', 'console', 'network', 'environment', 'annotations'];
      await Promise.all(
